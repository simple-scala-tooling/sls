...
object SmithyClientMain extends IOApp.Simple {
  ...
  def withTestWorkspace(f: => IO[?]): IO[Unit] =
    ...
    for {
      ...
      _          <- f.guarantee(state.parTraverse_ { case (path, bytes) =>
        fs2.io.file.Files[IO].writeAll(path).apply(Stream.emits(bytes)).compile.drain
      })
    } yield ()
  ...
  def run: IO[Unit] = withTestWorkspace {
    val run = for {
      ...
      classpath <- sys.env
        .get("TEST_CLASSPATH")
        .liftTo[IOStream](new Exception("TEST_CLASSPATH env var does not exists"))
      ...
      testScalaVersion <- sys.env
        .get("TEST_SCALA_VERSION")
        .liftTo[IOStream](new Exception("TEST_SCALA_VERSION env var does not exists"))
      result1 <- Stream.eval {
        server.compile(
          scopeId = "testModule",
          classpath = classpath.split(";").toList,
          sourcePath = sourcePath.split(";").toList,
          scalacOptions = List(""),
          javacOptions = List(""),
          scalaVersion = ScalaVersion(testScalaVersion),
        )
      }
      _ <- log(s"Client received RESULT 1 $result1")
      _ <- log("\n=================================================================================================\n")
      testObject <- sources0
        .find(_.toString.contains("TestObject"))
        .liftTo[IOStream](new Exception("Couldn't find TestObject.scala"))
      testObjectContent <- fs2.io.file.Files[IO].readUtf8(testObject)

      _ <- fs2.Stream.eval(
        fs2
          .Stream(testObjectContent.replace("%replaceMe%", "123"))
          .through(fs2.io.file.Files[IO].writeUtf8(testObject))
          .compile
          .drain
      )
      result2 <- Stream.eval {
      ...
      _ <- fs2.Stream.eval(
        fs2
          .Stream(testObjectContent.replace("%replaceMe%", "\"1234\""))
          .through(fs2.io.file.Files[IO].writeUtf8(testObject))
          .compile
          .drain
      )
