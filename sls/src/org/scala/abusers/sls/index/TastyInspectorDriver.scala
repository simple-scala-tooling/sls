...
import dotty.tools.dotc.fromtasty.{TASTYCompiler, TASTYRun, ReadTasty}
...
/** A low-level TASTy inspection driver that bypasses [[scala.tasty.inspector.TastyInspector]]
  * to allow reading both regular `.tasty` files and `.betasty` files produced by
  * best-effort compilation (via `-Ywith-best-effort-tasty`).
  *
  * Uses an internal [[dotty.tools.dotc.fromtasty.TASTYCompiler]] pipeline:
  * `ReadTasty` (frontend) followed by a custom phase that invokes the [[scala.tasty.inspector.Inspector]].
  */
object TastyInspectorDriver:
  ...
  private def inspect(files: List[String], dependenciesClasspath: List[String], bestEffort: Boolean)(inspector: Inspector): Boolean =
    files match
      case Nil => true
      case _ =>
