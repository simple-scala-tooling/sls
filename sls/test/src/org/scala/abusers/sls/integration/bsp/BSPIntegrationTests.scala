...
object BSPIntegrationTests extends LSPIntegrationTestSuite {
  ...
  def withBSPServer(workspace: Resource[IO, org.scala.abusers.sls.integration.utils.TestWorkspace]): Resource[IO, BSPTestContext] = {
    for {
      workspace     <- workspace
  ...
  test("supports Scala build target capabilities") { _ =>
    withBSPServer(org.scala.abusers.sls.integration.utils.TestWorkspace.withMultiModuleProject).use { ctx =>
      for {
        ...
        scalacOptions <- if (buildTargets.targets.nonEmpty) {
          bspServer.scala.buildTargetScalacOptions(
        ...
        expect(buildTargets.targets.nonEmpty) &&
  ...
  test("handles build target source discovery") { _ =>
    withBSPServer(org.scala.abusers.sls.integration.utils.TestWorkspace.withMultiModuleProject).use { ctx =>
      for {
        ...
        sources <- if (buildTargets.targets.nonEmpty) {
          bspServer.generic.buildTargetSources(
  ...
  test("handles concurrent BSP requests") { _ =>
    withBSPServer(org.scala.abusers.sls.integration.utils.TestWorkspace.withMultiModuleProject).use { ctx =>
      for {
        ...
        initRequest = bspServer.generic.buildInitialize(bsp.InitializeBuildParams(
